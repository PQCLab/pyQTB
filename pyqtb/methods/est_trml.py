"""
True rank maximum likelihood (TRML) estimator for POVM measurements results.
Density matrix of a pre-chosen rank is estimated by maximization of likelihood function.
The maximization is done by numerical solving of likelihood equation for the square root of a density matrix.
...
def est_trml(rank: int) -> EstimatorHandler:
    """TRML estimator
    ...
    def handler(meas: List[Measurement], data: List[np.ndarray], dim: Dimension) -> np.ndarray:
        ...
        return reconstruct_state(e, rank=rank, tol=1e-10, init=init).dm
